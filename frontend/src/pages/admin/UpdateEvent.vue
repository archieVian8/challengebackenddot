      ...
      <q-page>
        ...
        <div class="main-container">
          <div class="row items-center">
            ...
            <p class="jakarta-b text-3xl q-ml-md">Update Event Akademik</p>
          </div>
            ...
            <div class="col-6">
              ...
              <div class="row items-center justify-between q-mt-lg">
                <q-btn @click="this.$router.push('/organizer/event')" class="btn" outline color="grey-10" label="Kembali"
                  no-caps />
                <q-btn type="submit" class="btn" unelevated color="primary" label="Update Event" no-caps />
              </div>
            </div>
            <div class="col-5">
              <div class="q-mb-lg">
  ...
  methods: {
    async submit() {
      ...
      try {
        ...
        const response = await api.put(`/academicevents/update/${this.eventId}?idOrganizer=${organizerId}`, {
        ...
        Notify.create({
          color: 'green',
          message: 'Berhasil memperbarui event akademik',
          position: 'top',
          timeout: 2500
        });
    ...
    async fetchEventDetails() {
      try {
        const response = await api.get(`/academicevents/academicevents-id/${this.eventId}`);
        const event = response.data;
        this.name = event.eventsName;
        this.location = event.eventsLoc;
        this.jenjang = event.eventsJenjang;
        this.place = event.eventsHeld;
        this.category = event.eventCategory;
        this.eventDate = new Date(event.eventsDate).toISOString().split('T')[0];
        this.registdate = new Date(event.registrationDate).toISOString().split('T')[0];
        this.theme = event.eventsTheme;
        this.fee = event.registrationFee;
        this.capacity = event.capacityTotal;
        this.desc = event.deskripsiEvent;
      } catch (error) {
        Notify.create({
          color: 'red',
